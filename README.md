...
> A difference between classic *BJT* transistors (*PNP* and *NPN*, the upper line in the image above) and *FET* transistors (the lower line in above image) is that the latter show *four* instead of *three* terminals, one on the left side and three on the right side.
