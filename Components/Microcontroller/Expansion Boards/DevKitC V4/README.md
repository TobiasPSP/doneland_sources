...
*38-pin* development boards are especially well suited to experiment with all *ESP32S* features available as these development boards provide access to almost every microcontroller pin.
...
This *expansion board* is suited for most *38pin ESP32S* boards with a width of *24mm* or *25.5mm*. That's why there are *three* pin rows:

<img src="images/expansionboard_esp32_38pin_side_t.png" width="80%" height="80%" />


It was originally designed for the *ESP32 DevKitC V4* development board which has a width of *25.5mm* and plugs into the two outer pin rows, with the antenna facing outwards.
...
What sets this board apart from many others are the dedicated power supply pins per *GPIO*:

Each *GPIO* is exposed via a *yellow* male pin that is accompanied by a *red* and a *black* power supply pin. Being able to route power to any component right at the pin reduces wires and clutter.
...
Components you hook up to the board can either be *3.3V* or *5V* - which is why you can choose whether you want all of the *red* pins to either supply *3.3V* or *5V*. A jumper controls this voltage.
...
The board comes with a solid *800mA 5V* [LD1117C50](materials/ld1117_datasheet.pdf) voltage regulator that can be powered by a *USB-C* or *Micro-USB* connector, or a round *DC plug* and accepts input voltage up to *15V*.

#### Microcontroller Supply
The board supplies *5V* to the microcontroller, and draws power from its *3.3V* output pin. It feeds this voltage (produced by the microcontroller board) to its *3.3V power output pins*.
