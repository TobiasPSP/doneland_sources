...
* [**Voltage Drop**](Shunt): Every *load* consumes energy and produces a *voltage drop* proportional to the *current*. By inserting an *artificial load* in the form of a **Shunt** *resistor*, the *current* can be measured via the *voltage drop* at the known **Shunt** resistance. The **Shunt** must have (a) a *very precise* resistance, (b) a *very low* resistance, and (c) must be capable of handling the *total load current*.
