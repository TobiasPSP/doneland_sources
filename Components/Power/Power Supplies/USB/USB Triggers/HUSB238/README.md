...
Its unique feature is a *I2C* interface which is unusual in these entry-level triggering chips.
...
> [!IMPORTANT]
> *I2C* has always priority over the resistence configuration. So with the resistors, you set the baseline default. Once different settings are chosen via *I2C*, then these override the resistor settings.


#### Selecting USB Power Supply Setting
It is important to understand how trigger boards actually select a given voltage setting. Obviously, this depends on the actual capabilities of the USB power source.

* That's why *HUSB238* initially queries all the power profiles supported by the power source. 
* It then tries and finds the best matching profile for the requested voltage and current:
  * Highest source voltage available that is **equal or lower** to the requested voltage
  * Current is **equal or higher** to the requested current

That's why the real output voltage may fall back to a lower voltage than anticipated. 

For example, *12V* is no official *USB PD voltage*, however *12V* is supported by most chargers and power banks.

If however you come across a USB power source that works strictly by USB PD specification, then there may be no power profile for *12V*, just for *9V* and *15V*. In this case, *HUSB238* always selects a voltage **that is lower** than the requested voltage, and you end up with *9V output voltage*.
