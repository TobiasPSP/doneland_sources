...
> [!TIP]
> *DHCP* is great for consumer gear that you do not nned to address directly. To directly connect to a device, knowing its current *IP address* is mandatory. You cannot use a friendly and static *domain name* (like public websites do) because you do not run your own *DNS server* that would know how to translate your device names to *IP addresses*. That's why some *DIY Makers* manually assign *static IP addresses* to such devices: their *IP address* won't randomly change anymore and can now be used whenever you need to contact the device. A more sophisticated way uses *mDNS* (*Multicast DNS*) which is a decentralized *DNS* without the need for a dedicated *DNS server*. Instead, the devices themselves listen for *mDNS* multicast requests and provide their current IP address when asked to. *mDNS* only works within a *LAN*. A system in *one* network (i.e. a Raspberry Pi connected to a wired ethernet network) cannot resolve *mDNS names* for devices connected to *another* network (i.e. wireless devices connected to a *WiFi Router*.)
