...
* **MOSFET Transistors:** *do not require* a resistor at their control pin (*gate*).    
  ...
  However, the circuit that is *driving* the *gate pin* may need one. MOSFETs have a *high resistance*, so no high (continous) currents flow. 
  ...
  That's why for MOSFET, too, protective resistors are recommended (at least for larger MOSFETs), just for a different reason.
...
Distributors offer [online databases](https://www.mouser.de/c/semiconductors/discrete-semiconductors/transistors/mosfets/?transistor%20polarity=N-Channel&vgs%20th%20-%20gate-source%20threshold%20voltage=3.3%20V) for preselection. Once you found MOSFET types that appear suitable, ** always carefully manually review the datasheet** to see if it is really a fit. 
...
* **Vgs(th):** make sure the MOSFET starts to conduct well below the voltage you are intending to use. When *Vgs(th)* is close to, or above your voltage, the MOSFET is not suited. For *3.3 V*, choose choose `Vgs(th) <= 1.2V`. 
...
Before you order a MOSFET, **always carefully review its data sheet**. Never trust recommendations or online data bases. They can always point you to potentially interesting MOSFETs, but **you** must ultimately determine if a particular MOSFET is really a match **for your particular project**.

So with a shortlist of MOSFETs, you now need to look at each MOSFET data sheet and evaluate the fine print.

### On-State Resistance

A MOSFET **must turn on fully** when you apply a voltage to its *gate* pin. If it turns on only partially, it acts like a *resistor*, causing voltage drop and heat. The MOSFET could get extremely hot or become damaged.

#### Why Low Resistance Is Crucial
In real life, no MOSFET has a true zero Ohm resistance when *on*, but it is important to be close to zero Ohm, especially when switching high currents:

| Use Case | Maximum Acceptible Resistance |
| --- | --- |
...
* *Rds(on)*  is specified as *0.0085Ω*, which is very low, however at a gate voltage *Vgs=10V* of *10 V*.
* At *4.5 V*, *Rds(on)* already raises to *0.01Ω* which is still relatively low.

Unfortunately, most datasheets do not contain a *Rds(on)* **graph**. But they often provide a *Vgs Gate-to-Source Voltage* and label this *Transfer Characteristics*:

<img src="images/vgs_mosfet_example.png" width="100%" height="100%" />
...
The example graph shows a current of roughly *20 A* at *3.3 V*. Using Ohms Law, *Rds(on)* at *3.3 V* can be calculated:
...
* **MOSFET Driver:**    
  use a dedicated MOSFET driver IC. It accepts a logic level and outputs a higher voltage to drive the MOSFET gate.    
    ...
    MOSFET drivers operate a MOSFET it its perfect range but can be pricey.
...
| --- | --- | --- | --- | --- |
| SUP85N10-10-E3 | - | ++ | 6.00  | 60A, 100V |
| STP80NF03L-04 | + | +++ | - | 60A, 30V (*obsolete*) |
| IRLB8743PBF | + | ++ | 1.30| 78A, 30V |
| IRLIZ14GPBF | ++ | +++ | 2.10 |  6A, 60V |
| IRLI640GPBF | + | ++ | 2.38 | 8A, 200V |
| IRLIZ44GPBF | - | + | 3.29 | 30A, 60V |
| RX3G07CGNC16 | + | +++ | 2.30 | 70A, 40V |
| FDP5800 | + | ++ | 2.80 | 80A, 60V |
| FDPF320N06L | - | + | 2.30 | 21A, 60V |
| NDP6060L | + | ++ | 3.50 | 48A, 60V |
| VN3205N3-G | - | + | 1.60 | 1.2A, 50V |
| IPP80N03S4L-03 | - | ++ | 3.00 | 80A, 30V |
| STD12NF06L-1 | - |+++ | 0.97 | 12A, 60V |
| STP55NF06L | - | ++ | 2.00 | 55A, 60V |
| SUP85N10-10-GE3 | - | + | 6.00 | 85A, 100V |
| HUF76423P3 | + | +++ | 1.55 | 20A, 60V |
