...
>
> On top, many cheap breakout boards use counterfeit controllers that may still work for you but have significantly different specs. For example, some counterfeit **XL4005** do not run at 300kHz switching frequency but rather 50kHz. If the core of the inductor on the board is colored *yellow-white* (like on the one depicted above), then it might be a core with a maximum frequency of around 70kHz (real **XL4005** run at 300kHz) made from iron that is originally designed to be used in filters, not in *DC-DC converters*.
>
