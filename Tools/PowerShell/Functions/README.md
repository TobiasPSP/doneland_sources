...
> [!TIP]
> My notebook originally did not qualify for a *Windows 11 upgrade* despite being from *2018*. I then learned that while there are many items of the prerequisite check that make sense, most older computers are hit by *CPU requirements* that simply don't seem to make sense. Even an Intel i5 with planty of power is (officially) not supported by *Windows 11*. I then used [a PowerShell script](https://github.com/Ad3t0/DirectWindowsUpgrade) to successfully upgrade my old *i5* notebook to *Windows 11*, and it runs smoothly ever since. Just make sure you download the *Windows 11 ISO Image* that matches your current *Windows 10 edition*, or else your existing license will not be transitioned, and you need a new one.  
